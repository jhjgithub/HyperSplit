...
{
	if (p_pnt) {
		printf("Point: %016" PRIx64 "%016" PRIx64 "\n",
...
void gen_prefix_mask(union point *p_out, unsigned int bits, unsigned int mask_len);
void gen_suffix_mask(union point *p_out, unsigned int mask_len);

int prefix2range(struct range *p_range, const struct prefix *p_prefix, unsigned int bits);
int range2prefix(struct prefix_vector *p_vector, const struct range *p_range, unsigned int bits);
